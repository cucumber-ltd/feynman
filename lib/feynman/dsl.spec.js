...
* button`
  ...
  describe('tasks', () => {
    let handleCreateUserNamed, dave

    beforeEach(async () => {
      handleCreateUserNamed = sinon.spy()
    })
    ...
    context('a task with no parameters', () => {
      ...
      beforeEach(async () => {
        perspective('web', handle => {
          handle(CreateUser, () => () => handleCreateUserNamed())
        })
        perspective.default('web')
        dave = await actor('dave')
      })
      ...
      it('runs the perspective handler when attempted', async () => {
        await dave.attemptsTo(CreateUser)
        sinon.assert.called(handleCreateUserNamed)
      })
    ...
    context('a task with one parameter', () => {
      const { CreateUser } = tasks(task => ({
        CreateUser: {
      ...
      beforeEach(async () => {
        perspective('web', handle => {
          handle(CreateUser.named, ({ name }) => () => handleCreateUserNamed(name))
        })
        perspective.default('web')
        dave = await actor('dave')
      })
      ...
      it('runs the perspective handler when attempted', async () => {
        ...
        sinon.assert.calledWith(handleCreateUserNamed, 'dave')
      })
