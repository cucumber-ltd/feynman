...
describe('feynman DSL', () => {
  ...
  describe('defining tasks', () => {
    ...
    it('uses the class and method names to infer a description', () => {
      const { CreateUser } = tasks({
        CreateUser: {
          named: ['name'],
        },
      })
      assert.deepEqual(CreateUser.named.description, 'Create user named')
    })

    it('creates a deeply nested task with parameters', () => {
      const { CreateUser } = tasks({
        CreateUser: {
          named: [
            'name',
      ...
      assert.deepEqual(
        CreateUser.named.withBacon.id,
        'CreateUser.named.withBacon'
      )
    })
  })
})
