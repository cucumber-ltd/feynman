...
const interactions = getDefinition => {
  const handle = handler => {
    if (typeof handler !== 'function') throw new Error('A handler must be a function')
  ...
  const describeHandlersInObject = (definition, parentDescription) =>
    ProvideHintsToUser(
      parentDescription,
  ...
  return describeHandlersInObject(definition)
}
