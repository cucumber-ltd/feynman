    ...
    asksFor: (...questions) => ({
      andVerify: async fn => {
        ...
        await new Promise(async (resolve, reject) => {
          let lastErr
          //TODO: Remove timeout when no inquisitors and configure timeout for inquisitors.
          const timeout = setTimeout(async () => {
            await unsubscribeAll()
            reject(
              //TODO: Add more information about pending unanswered questions
              lastErr ||
                new Error("Didn't get an acceptable answer after 500ms")
            )
          }, 500)
  ...
  const perspective = {
    ...
    handlerFor: ({ id, params = {} }) => {
      if (!handlers.hasOwnProperty(id))
        throw new Error(
          `No handler found for task '${id}' in '${name}' perspective.\n\n${
