...
describe('feynman core objects', () => {
  ...
  describe('@wip questions', () => {
    ...
    it('can yield simple answers', async () => {
      const perspective = Perspective('test', handle => {
        handle(ASimpleQuestion, () => () => Promise.resolve(answer))
      })
      await Actor({}, perspective)
        .asksFor(ASimpleQuestion)
        .andVerify(aSimpleAnswer => assert.equal(aSimpleAnswer, answer))
    })
    ...
    it('can ask several questions', async () => {
      ...
      const perspective = Perspective('test', handle => {
        handle(ASimpleQuestion, () => () => Promise.resolve(answer))
        handle(AnotherSimpleQuestion, () => () =>
          Promise.resolve(anotherAnswer)
        )
      })
      await Actor({}, perspective)
