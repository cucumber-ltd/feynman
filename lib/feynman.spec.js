 ...
 * Director - context within which an actor acts, it knows how to perfom
...
const Director = (actions, abilities) => actor => {
  const director = {
    attemptsTo: action => {
      action({ director, actor: actor.withAbilities(abilities) })
    },
    makeThem: task => {
      director.attemptsTo(actions[task]),
    }
  }
  return director
}
...
describe('feynman', () => {
  // Tasks
  const Book = {
    aRoom: ({ director }) => director.makeThem(Book.aRoom),
    aFlight: ({ director }) => director.makeThem(Book.aFlight)
    aHoliday: ({ director }) => {
      director.attemptsTo(Book.aRoom)
      director.attemptsTo(Book.aFlight)
    },
  }
  ...
  it('allows an actor to have multiple perspectives', () => {
    ...
    const throughTheDomain = Director(DomainActions, { domain })
    const throughTheWebApp = Director(WebAppActions, {
      browser,
    })
