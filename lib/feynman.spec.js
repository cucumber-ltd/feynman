...
const { Perspective, Actor, Task, Memory, Remember } = require('./feynman')
...
describe('feynman', () => {
  describe('task definitions', () => {
    ...
    it('@wip deeply nests tasks without calling the root task', () => {
  ...
  context('handling tasks through perspectives', () => {
    it('@wip attempts a task with no parameters', async () => {
      const browser = {
        refresh: sinon.spy(),
      }
    ...
    it('@wip attempts a task with a single parameter', async () => {
      const browser = {
        clickOn: sinon.spy(),
      }
      const abilities = { browser }
    ...
    it('@wip attempts a nested task', async () => {
      const browser = {
        fillIn: sinon.spy(),
      }
      const abilities = { browser }
  ...
  context('@wip with multiple perspectives', () => {
    ...
    it('allows actors to attempt the same task from different perspectives', async () => {
      ...
      const webAppJoe = Actor(webAppPerspective, { browser, state })
  ...
  describe('Memory', () => {
    it('allows an actor to remember state', async () => {
      ...
      await joe.attemptsTo(Remember('thing', 'something'))
      assert.equal(await joe.asks(Memory.about('thing')), 'something')
    })
  })
})
