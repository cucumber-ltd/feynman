...
const { Perspective, Actor, Task, Memory } = require('./feynman')
...
describe('feynman', () => {
  context('handling tasks through perspectives', () => {
    ...
    it('attempts a task with named parameters', async () => {
      const browser = {
        fillIn: sinon.spy(),
      }
      const abilities = { browser }
      const FillIn = { label: Task() }
      const perspective = Perspective(handle => {
  ...
  describe('Memory', () => {
    it('allows an actor to remember state', async () => {
      const doSomethingToRemember = Task()
      const doSomethingThatRecallState = Task()
      const perspective = Perspective(handle => {
        handle(doSomethingToRemember, () => ({ remember }) =>
          remember({ something: 'something' })
        )
        handle(doSomethingThatRecallState, () => ({ recall }) =>
          assert.equal(recall('something'), 'something')
        )
      })
      const joe = Actor(perspective, { ...Memory() })
      await joe.attemptsTo(doSomethingToRemember, doSomethingThatRecallState)
    })
  })
})
