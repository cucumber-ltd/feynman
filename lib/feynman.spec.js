...
const Script = (scriptName, ...sceneNames) => {
  const script = {}

  script.browser = ({ id: 'Refresh.browser', args: [] })
  script.browser.id = 'Refresh.browser'

  script.button = (...args) => ({ id: 'Click.button', args })
  script.button.id = 'Click.button'

  const makeChild = (...args) => {
    const script = {}
    script.with = (...args) => ({ id: 'FillIn.label.with', args })
    script.id = 'FillIn.label.with'
    return script
  }
  script.label = makeChild
  script.label.id = 'FillIn.label'
  script.label.with = makeChild() // TODO: can't quite figure this bit out....?
  script.label.with.id = 'FillIn.label.with'
...
const perspective = describe('feynman', () => {
  context('an action with no parameters', () => {
    it('is attempted by the actor', () => {
      ...
      const Refresh = Script('Refresh', 'browser')
      const perspective = Perspective(Action => {
        Action(Refresh.browser, text => ({ browser }) => browser.refresh())
      })
  ...
  context('an action with a single parameter', () => {
    it('is attempted by the actor', () => {
      ...
      const abilities = { browser }
      const ClickOn = Script('ClickOn', 'button')
      const perspective = Perspective(Action => {
        Action(ClickOn.button, text => ({ browser }) => browser.clickOn(text))
      })
      const joe = Actor(perspective, abilities)
  ...
  context('an action with multiple parameters', () => {
    it('is attempted by the actor', () => {
      ...
      const abilities = { browser }
      const FillIn = Script('FillIn', 'label', 'with') // TODO: decide what API we like here, this doesn't feel right
      const perspective = Perspective(Action => {
        Action(FillIn.label.with, label => text => ({ browser }) => browser.fillIn(label, text))
      })
      const joe = Actor(perspective, abilities)
      joe.attemptsTo(FillIn.label('email').with('joe@example.com'))
